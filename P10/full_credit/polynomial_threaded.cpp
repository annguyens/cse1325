...
void Polynomial::solve(double min, double max, int nthreads, double slices, double precision) {
    _roots = {};
	double increment = (max-min)/nthreads;
	double max2 = increment;
	std::thread t[nthreads];

	for(int i=0;i<nthreads;i++)
	{
		//t[i]= std::thread{&Polynomial::solve_recursive, this, min, max2, 1, slices, precision};
		t[i]= std::thread([this, &min,&max2,&slices,&precision](){solve_recursive(min,max2,1,slices,precision);});
		min=max2;
		max2+=increment;
	}
